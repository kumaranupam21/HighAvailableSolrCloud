param (
    ...
    [Parameter(Mandatory=$True)]
    [ValidateNotNullOrEmpty()]
    [string] $virtualNetworkAddressPrefixes,
        

    [Parameter(Mandatory=$False)]
    [ValidateNotNullOrEmpty()]
    [string] $storageAccountNameAfterPrefix = "slrlrs01"
)
...
New-AzResourceGroupDeployment -verbose `
                              ...
                              -adminUsername (ConvertTo-SecureString $adminUsername -AsPlainText -Force) `
...
Write-Host "getRGDeploy: " $getRGDeploy

$outputParameterServerNames = $getRgDeploy.Outputs.serverName.Value
$allServers = $outputParameterServerNames.ToString() | ConvertFrom-Json

$serverName = $null;
if ($null -eq $outputParameterServerNames) {
...
Write-Host "serverNames: " $serverNames
